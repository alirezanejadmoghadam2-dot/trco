...
async def health_check():
    ...
    if bot_task and not bot_task.done():
        ...
        # اگر تسک اصلی به هر دلیلی تمام شده باشد، این به شما در عیب‌یابی کمک می‌کند
...
# ==============================================================================
# بخش ۴: تابع تست خودکار (با سفارش Limit و انتظار فعال)
# ==============================================================================
async def run_startup_test():
    ...
    try:
        ...
        print(f"این سفارش به مدت {wait_seconds} ثانیه باز خواهد ماند. در این مدت برای فعال نگه داشتن سرویس، قیمت چک می‌شود.")
        
        end_time = asyncio.get_event_loop().time() + wait_seconds
        while asyncio.get_event_loop().time() < end_time:
            try:
                ticker = await exchange.fetch_ticker(test_symbol)
                print(f"   (تست در حال اجرا... قیمت فعلی: {ticker['last']})")
            except Exception as e:
                print(f"   (خطا در دریافت قیمت حین تست: {e})")
            await asyncio.sleep(30)
...
async def startup_event():
    ...
    bot_task = asyncio.create_task(run_main_logic())

@app.on_event("shutdown")
async def shutdown_event():
    """در هنگام خاموش شدن، تسک ربات را لغو می‌کند."""
    if bot_task:
        bot_task.cancel()
        print("🛑 تسک ربات لغو شد.")
