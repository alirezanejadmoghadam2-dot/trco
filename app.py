...
# ==============================================================================
# بخش ۲: توابع تحلیل تکنیکال (مغز ربات)
# ==============================================================================
...
# *** تابع اصلاح شده برای رفع هشدار ***
def upsert_last_candle(df_all: pd.DataFrame, last_candle_df: pd.DataFrame) -> pd.DataFrame:
    if last_candle_df is None or len(last_candle_df) == 0:
        return df_all
    last_new = last_candle_df.iloc[-1]
    if len(df_all) == 0:
        return last_candle_df.copy()
    
    if int(last_new["time"]) > int(df_all.iloc[-1]["time"]):
        return pd.concat([df_all, last_candle_df], ignore_index=True)
    else:
        common_cols = df_all.columns.intersection(last_new.index)
        df_all.loc[df_all.index[-1], common_cols] = last_new[common_cols]
        return df_all
